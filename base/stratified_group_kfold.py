...
if __name__ == "__main__":
    train_x = pd.read_csv("../data/raw/train.csv")
    train_y = train_x.sirna.values
    groups = np.array(train_x.experiment.values)
    ...
    result =  pd.DataFrame(distrs, index=index, columns=[f'Label {l}' for l in range(np.max(train_y) + 1)])
    print(result)
